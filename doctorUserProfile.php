...
if (isset($_POST[md5("savePatientProfileInformation")])) {
    $uploaddir = './assets/img/doctors/'.session_id().date("y-m-d-h-i-s");
...
<script src='https://cdn.jsdelivr.net/npm/promise-polyfill'></script>
           ...
           Swal.fire({
            ...
            confirmButtonText: 'Okay'
             ...
             setTimeout(()=>{
                 window.location.href='./doctorUserProfile.php';
             },10);
          } )</script></body>";
    }
}
                        ...
                        if ($row = $result->fetch_assoc())                       
							...
							<div class="card">
								<div class="card-body">
									...
									<form id="patientProfileForm" action="" enctype="multipart/form-data" method="POST">
                                        ...
                                        <input required hidden type="password" value="' . md5(session_id()) . '" name="' . session_id() . '">
											<button  type="button" style="margin:4px;background-color:#757575;border: 1px solid #757575;" class="btn btn-primary submit-btn"><a style="color:white" href="./DoctorProfile.php?' . md5(session_id() + "id") . '=' . $_SESSION["user_id"] . '">View Public Profile</a></button>
											<button style="margin:4px" type="submit" onclick="event.preventDefault(); validateInputFields();" class="btn btn-primary submit-btn">Save Changes</button>
										</div>
									</form>
									<!-- /Profile Settings Form -->
								</div>
