...
if (isset($_POST[session_id()])) {
	if ($_POST[session_id()] == md5(session_id())) {
		if (isset($_POST["first_name"]) && isset($_POST["last_name"]) && isset($_POST["email"]) && isset($_POST["mobile"]) && isset($_POST["password"]) && isset($_POST["confirm_password"]) && isset($_POST["address"]) && isset($_POST["dob"]) && isset($_POST["gender"])) {
			...
			$specialities = addslashes(trim($_POST["specialities"]));
            $cost = addslashes(trim($_POST["cost"]));
			$business_hours = addslashes(trim($_POST["business_hours"]));

			if ($password == $confirm_password) {
				$password = md5(md5($key . addslashes(trim($password)) . $key));
				
				$sql = "INSERT INTO user_profile(title, description, specialities, cost, business_hours, feedback_id) VALUES ('$title', '$description', '$specialities', '$cost', '07:00 AM - 09:00 PM, 07:00 AM - 09:00 PM, 07:00 AM - 09:00 PM, 07:00 AM - 09:00 PM, 07:00 AM - 09:00 PM, 07:00 AM - 09:00 PM,Closed','1')";
				...
				if ($result === TRUE) {
					...
					$sql = "INSERT INTO users(user_type_id, user_profile_id, first_name, last_name, email, password, mobile, address, gender, dob, status) VALUES (2, '$user_profile_id', '$first_name', '$last_name', '$email', '$password', '$mobile', '$address', '$gender', '$dob', '1')";
					$result = $conn->query($sql);
...
<script src='https://cdn.jsdelivr.net/npm/promise-polyfill'></script>
			...
			// $sql = "INSERT INTO user_profile(title, description, specialities, cost) VALUES ('$title', '$description', '$specialities', '$cost')";
								...
								<div class="col-md-12 col-lg-6 login-right">
									...
									<form id="registrationForm" action="" method="POST">
										...
										<div class="row">
											<div class="col-md-12 col-lg-6">
												<div class="form-group form-focus focused">
													<input id="title" name="title" required type="text" class="form-control floating">
													<label class="focus-label">Title</label>
												</div>
											</div>
											<div class="col-md-12 col-lg-6">
												<div class="form-group form-focus focused">
													<input id="description" name="description" required type="text" class="form-control floating">
										...
										<div class="row">
											<div class="col-md-12 col-lg-6">
												<div class="form-group form-focus focused">
													<input id="specialities" name="specialities" required type="text" class="form-control floating">
													<label class="focus-label">Specialities</label>
												</div>
											</div>
											<div class="col-md-12 col-lg-6">
												<div class="form-group form-focus focused">
													<input id="cost" name="cost" required type="number" class="form-control floating">
